...
contract InstaETHVaultV2Resolver is Helpers {
    ...
    struct ProtocolInfo {
        ...
        ProtocolAssetsWstETH fluid;
        ProtocolAssetsWstETH fluidNew;
        ProtocolAssetsWstETH aaveV3Lido;
    }
        ...
        )
    {
        ...
        protoInfo_.fluid.protocolId = 8;

        (
            protoInfo_.fluid.wstETHCol,
            protoInfo_.fluid.stETHCol,
            protoInfo_.fluid.wETHDebt,
            protoInfo_.fluid.ratio // (WETH / STETH)
        ) = VAULT_V2.getRatioFluid(stEthPerWsteth_);

        protoInfo_.fluid.maxRiskRatio = VAULT_V2.maxRiskRatio(
            protoInfo_.fluid.protocolId
        );

        (
            protoInfo_.fluid.rates.wstETHSupplyRate,
            protoInfo_.fluid.rates.wETHBorrowRate
        ) = getFluidRates();

        protoInfo_.fluid.rates.stETHSupplyRate = convertWstethRateForSteth(
            protoInfo_.fluid.rates.wstETHSupplyRate,
            stEthPerWsteth_
        );
        ...
        protoInfo_.fluidNew.protocolId = 9;

        (
            protoInfo_.fluidNew.wstETHCol,
            protoInfo_.fluidNew.stETHCol,
            protoInfo_.fluidNew.wETHDebt,
            protoInfo_.fluidNew.ratio // (WETH / STETH)
        ) = VAULT_V2.getRatioFluidNew(stEthPerWsteth_);

        protoInfo_.fluidNew.maxRiskRatio = VAULT_V2.maxRiskRatio(
            protoInfo_.fluidNew.protocolId
        );

        (
            protoInfo_.fluidNew.rates.wstETHSupplyRate,
            protoInfo_.fluidNew.rates.wETHBorrowRate
        ) = getFluidNewRates();

        protoInfo_.fluidNew.rates.stETHSupplyRate = convertWstethRateForSteth(
            protoInfo_.fluidNew.rates.wstETHSupplyRate,
            stEthPerWsteth_
        );
        ...
        protoInfo_.aaveV3Lido.protocolId = 10;

        (
            protoInfo_.aaveV3Lido.wstETHCol,
            protoInfo_.aaveV3Lido.stETHCol,
            protoInfo_.aaveV3Lido.wETHDebt,
            protoInfo_.aaveV3Lido.ratio // (WETH / STETH)
        ) = VAULT_V2.getRatioAaveV3Lido(stEthPerWsteth_);

        protoInfo_.aaveV3Lido.maxRiskRatio = VAULT_V2.maxRiskRatio(10);

        (
            protoInfo_.aaveV3Lido.rates.wstETHSupplyRate,
            protoInfo_.aaveV3Lido.rates.wETHBorrowRate
        ) = getAaveV3LidoRates();

        protoInfo_.aaveV3Lido.rates.stETHSupplyRate = convertWstethRateForSteth(
            protoInfo_.aaveV3Lido.rates.wstETHSupplyRate,
            stEthPerWsteth_
        );
    }
    ...
    struct UIInfo {
        ...
        uint256 availableFluid;
        uint256 availableFluidNew;
        uint256 availableAaveV3Lido;
    ...
    function getUIDetails(
        ...
        uiInfo_.availableWithdrawTotal =
            ...
            uiInfo_.availableFluid +
            uiInfo_.availableFluidNew +
            uiInfo_.availableAaveV3Lido +
    ...
    function availableWithdrawInProtocols(
        ...
        // in 1e6
        uint256 ratioDiffFluid_ = uiInfo_.protoInfo.fluid.ratio <
            uiInfo_.protoInfo.fluid.maxRiskRatio // we can set buffer margin on backend.
            ? uiInfo_.protoInfo.fluid.maxRiskRatio -
                uiInfo_.protoInfo.fluid.ratio
            : 0;

        // in 1e6
        uint256 ratioDiffFluidNew_ = uiInfo_.protoInfo.fluidNew.ratio <
            uiInfo_.protoInfo.fluidNew.maxRiskRatio // we can set buffer margin on backend.
            ? uiInfo_.protoInfo.fluidNew.maxRiskRatio -
                uiInfo_.protoInfo.fluidNew.ratio
            : 0;

        // in 1e6
        uint256 ratioDiffAaveV3Lido_ = uiInfo_.protoInfo.aaveV3Lido.ratio <
            uiInfo_.protoInfo.aaveV3Lido.maxRiskRatio // we can set buffer margin on backend.
            ? uiInfo_.protoInfo.aaveV3Lido.maxRiskRatio -
                uiInfo_.protoInfo.aaveV3Lido.ratio
            : 0;
        ...
        uiInfo_.availableFluid = uiInfo_.protoInfo.fluid.maxRiskRatio == 0
            ? uiInfo_.protoInfo.fluid.stETHCol
            : (uiInfo_.protoInfo.fluid.stETHCol * ratioDiffFluid_) /
                uiInfo_.protoInfo.fluid.maxRiskRatio;

        uiInfo_.availableFluidNew = uiInfo_.protoInfo.fluidNew.maxRiskRatio == 0
            ? uiInfo_.protoInfo.fluidNew.stETHCol
            : (uiInfo_.protoInfo.fluidNew.stETHCol * ratioDiffFluidNew_) /
                uiInfo_.protoInfo.fluidNew.maxRiskRatio;

        uiInfo_.availableAaveV3Lido = uiInfo_.protoInfo.aaveV3Lido.maxRiskRatio == 0
            ? uiInfo_.protoInfo.aaveV3Lido.stETHCol
            : (uiInfo_.protoInfo.aaveV3Lido.stETHCol * ratioDiffAaveV3Lido_) /
                uiInfo_.protoInfo.aaveV3Lido.maxRiskRatio;
