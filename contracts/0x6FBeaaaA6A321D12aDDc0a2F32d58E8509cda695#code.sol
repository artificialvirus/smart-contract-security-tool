...
//This smart contract is generated by http://omni.fun. Please visit https://omni-trade.io for trade.
...
// File contracts/interfaces/ICrossChainToken.sol
...
interface ICrossChainToken {
    ...
    /// @dev Throw if token is not taxable
    error NotTaxableToken();
    /// @dev Throw if token dnt support whitelist
    error NotWhitelistableToken();
    /// @dev Throw if token dnt support blacklist
    error NotBlacklistableToken();
    /// @dev Throw if wallet is blacklisted
}


// File contracts/NewCrossChainToken.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity 0.8.23;
/// @title Cross Bridge OFT token
contract CrossBridgeToken is OFT, ICrossChainToken {

    uint32 public constant mainCrossRelayerId = 30184;
    ...
    constructor(
        ...
        bytes32 format = addressToBytes32(_mainToken);
        _setPeer(mainCrossRelayerId,format);
        setCrossConnection(mainCrossRelayerId);
    ...
    function setSingleConnection(uint32 _mainCrossRelayerId, address _crossTokenAddress) public onlyOwner { 
        bytes32 format = addressToBytes32(_crossTokenAddress);
        _setPeer(_mainCrossRelayerId,format);
        setCrossConnection(_mainCrossRelayerId);
    }

    function setCrossConnection(uint32 _mainCrossRelayerId) internal {
        EnforcedOptionParam[] memory subMsg = new EnforcedOptionParam[](2);
        EnforcedOptionParam memory msg1 = EnforcedOptionParam({
            eid: _mainCrossRelayerId,
            msgType: uint16(1),
            options: hex"00030100110100000000000000000000000000030d40"
        });
        EnforcedOptionParam memory msg2 = EnforcedOptionParam({
            eid: _mainCrossRelayerId,
            msgType: uint16(2),
            options: hex"00030100110100000000000000000000000000030d4001001303000000000000000000000000000000030d4001001303000100000000000000000000000000030d40"
        });

        subMsg[0] = msg1;
        subMsg[1] = msg2;

        _setEnforcedOptions(subMsg);
